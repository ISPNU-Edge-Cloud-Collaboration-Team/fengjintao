...
我们需要大模型提供的能力：根据我对输入的限制，生成的模型，对生成模型的要求不是很高，以为后续可以通过判别器筛选。

选用的判别器：NAR-Former V2 延迟用的预测集是NNLOP中的，精度预测用的是NAS-Bench-101 和NAS-Bench-201 （精度评估预测序列与真实的序列之间的相关性）。

![总体结构最新版](一些图片/总体结构最新版.png)

1.大语言模型生成模型架构的训练方式，如何设置数据集？
