...
​     与NAS中不限制延迟的方法比较我们的需要是需要限制延迟因此与NAS中对延迟进行限制的方法比较，

​      DGL: Device Generic Latency Model for NeuralArchitecture Search on Mobile Devices，BRP-NAS: Prediction-based NAS using GCNs。

​      NAS的方法中得到延迟约束下的最佳网络架构需要执行一次搜索，而我们找到的网络不是最佳架构但是耗费时间低于NAS，一边是执行一遍搜索得到结果，一边是模型进行一遍推理。NAS的延迟约束是低于给定延迟，是否有场景是需要给定延迟。比如：生产线上对产品进行检测，检测时间与流水线速度相关，速度过快过慢会造成不同步的问题。

​      生成式模型方面：Learning Where To Look –Generative NAS is Surprisingly Efficient：在查询效率和寻找最有前途的架构之间的权衡下用生成模型在生成器从中提取的潜在空间内进行基于优化的搜索。Large Language Models Synergize with Automated Machine Learning：论文提出了用文本描述生成模型。此类方法不能找到指定延迟和精度的模型。

​      现有的方法，在给定延迟和精度输出相应要求模型的任务中都存在缺陷。大模型在预训练期间学习了有关模型算子相关的代码，我们将模型结构和延迟精度对应的先验知识让大模型进行微调。

​      用大模型做NAS的论文：Can GPT-4 Perform Neural Architecture Search?利用GPT4作为黑盒优化器的生成能力，快速导航架构搜索空间，查明有前途的候选人。
...
![0d5cf95ada30e6d17ba74d458aa7d54](一些图片/0d5cf95ada30e6d17ba74d458aa7d54.png)

![47f3162b331d51e91e9a242978ea005](一些图片/47f3162b331d51e91e9a242978ea005.png)

test pred_cost tensor([[1.9064]], device='cuda:0') tensor([[0.8123]], device='cuda:0')

test pred_cost tensor([[0.5031]], device='cuda:0') tensor([[1.0126]], device='cuda:0')

test pred_cost tensor([[1.9064]], device='cuda:0') tensor([[0.6219]], device='cuda:0')
...
| ---------- | -------------- | ------ |
|            |                |        |
| 0.5141     | 0.0126         | 0.5015 |
| 1.9064     | 0.6219         | 1.2845 |
| 0.4471     | 1.6200         | 1.1729 |
| 0.4431     | 0.7000         | 0.2571 |
| 6.5756     | 6.1200         | 0.4556 |
| 1.9064     | 0.8123         | 1.0941 |
| 0.5031     | 1.0126         | 0.5095 |
| 6.5704     | 10.4000        | 3.8296 |
| 13.0174    | 20.14          | 7.1226 |
| 10.01      | 15.64          | 5.63   |
| 13.0174    | 20.12          | 7.1026 |
| 9.54       | 17.89          | 8.35   |
| 9.9018     | 9.01           | 0.8908 |
| 8.62       | 5.67           | 2.95   |
| 10.35      | 10.12          | 0.23   |
...
1.生成的模型放入nar判别器中存在无法适配的情况

2.在实际部署中的onnx需要trt加速，不用trt加速直接执行onnx模型存在报错 现在判断是ONNXRuntime版本的问题

3.自动根据大模型的输出生成onnx有问题（温老师的意思是不使用手写脚本的方法，考虑用大模型解决）
...
| **要求大模型达到的****延迟** | **NAR****判别的****输出** | **MAC**       | **绝对百分比误差**** ** | ** ** | ** ** |
| ---------------------------- | ------------------------- | ------------- | ----------------------- | ----- | ----- |
| 3.1211                       | 4.9465                    | 816,129,415   | 0.584857903             |       |       |
| 3.2200                       | 3.0920                    | 2,656,022,078 | 0.039751553             |       |       |
| 3.5411                       | 5.8842                    | 2,247,643,738 | 0.661687046             |       |       |
| 3.6584                       | 4.6991                    | 532,225,379   | 0.28446862              |       |       |
| 3.9854                       | 2.3428                    | 486,338,251   | 0.412154363             |       |       |
| 4.0566                       | 7.9840                    | 1,131,900,048 | 0.968150668             |       |       |
| 4.5600                       | 4.3570                    | 571,605,285   | 0.024320175             |       |       |
| 5.6122                       | 5.1431                    | 3,043,252,209 | 0.08358576              |       |       |
| 6.1200                       | 5.8250                    | 3,317,678,176 | 0.048202614             |       |       |
| 6.5129                       | 6.3639                    | 3,649,300,032 | 0.022877674             |       |       |

MAPE=0.31

NAR判别器中的对Alexnet网络判别的MAPE为0.24



5.28办公室交流：
1. 解决其他类型模型的重构问题，处理复杂连接。
2. 生成多种类型的模型，大模型微调中加入模型类型的提示词，比如给我生成alexnet模型、mobilenetv模型、vgg等
3. 微调的数据集中加入精度，可以用Prompt或者其他方式告诉大模型哪些算子或者框架对精度提升有用。
4. 在实际边缘设备中检测延迟和精度，如果要考虑实际设备，需要将数据集中的模型在实际边缘设备中测试延迟。
5. 在大模型生成之后加上一个模块用来提升精度。
...
​       处理复杂连接关系的编码解码策略：大模型训练过程中把每一个input和output进行id编码（一个id对应一个维度），在重构的时候先搜索一遍网络把id和对应的维度存储为一个字典，遇到add等算子需要多输入时检索字典找到对应的id。

​      进行id编码策略的改动的影响： 1.大模型的输出语法可能会影响

​                                                            2.对应改动之后生成模型的代码也需要改

​                                                            3.转化为onnx模型之后是否能输入NAR判别器
...
NAR中使用NAS201数据进行预测精度的中读取数据的内容中不包括网络的权重信息
读取了网络结构、性能参数（精度、平均精度）
精度策略：
1.自己做一个动态编码可以对大模型输出的模型进行编码，将编码方式用在NAS数据集中重新训练NAR判别器（会对NAR的精度有影响，但是可以对大模型的输出进行）

2.直接在实际边缘设备上进行模型训练测试精度，如果精度不高则通过其他方法提升精度
